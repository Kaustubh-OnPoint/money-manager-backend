...
public class NotificationService {
    ...
    @Scheduled(cron = "0 0 10 * * 0", zone = "IST")
    public void sendWeeklySummary() {
        log.info("Job started: sendWeeklySummary()");
        LocalDate weekStart = LocalDate.now().minusDays(6);
        LocalDate weekEnd = LocalDate.now();
        List<ProfileEntity> profiles = profileRepository.findAll();
        for (ProfileEntity profile : profiles) {
            BigDecimal weekIncome = incomeRepository.sumAmountByProfileIdAndDateBetween(profile.getId(), weekStart, weekEnd);
            weekIncome = weekIncome != null ? weekIncome : BigDecimal.ZERO;
            BigDecimal weekExpense = expenseRepository.sumAmountByProfileIdAndDateBetween(profile.getId(), weekStart, weekEnd);
            weekExpense = weekExpense != null ? weekExpense : BigDecimal.ZERO;
            BigDecimal netSavings = weekIncome.subtract(weekExpense);

            if (weekIncome.compareTo(BigDecimal.ZERO) > 0 || weekExpense.compareTo(BigDecimal.ZERO) > 0) {
                String body = "Hi " + profile.getFullName() + ",<br><br>"
                        + "<b>Your Weekly Financial Summary</b><br>"
                        + "Week: " + weekStart + " to " + weekEnd + "<br><br>"
                        + "Total Income: &#8377;" + weekIncome + "<br>"
                        + "Total Expense: &#8377;" + weekExpense + "<br>"
                        + "Net Savings: &#8377;" + netSavings + "<br><br>"
                        + "Best regards,<br>Money Manager Team";
                emailService.sendEmail(profile.getEmail(), "Your Weekly Financial Summary", body);
            }
        }
        log.info("Job completed: sendWeeklySummary()");
    }
}
